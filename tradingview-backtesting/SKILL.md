...
- **The Strategy Tester has returned byte-identical results across genuinely different inputs after ~3–4 rapid recalculations.** [REPORTED operator heuristic — recurrent, never a controlled observation] Distinguish carefully: adjacent parameter cells *should* give similar numbers; frozen means *identical*. Before restarting Chrome, verify script header, inputs, and one deliberately result-changing control.
...
1. Watchdog running (`tv-tab-watchdog.sh`); chart tab active (`document.hidden === false`).
...
5. **Realized round-turn cost confirmed in the List of Trades** before trusting the batch.
