...
"""Score a falsify report against the fixture's answer key.
  ...
  score.py REPORT.md [--key answer-key.json] [--json]
...
def mentions(text, files, keywords):
    ...
    return any(any(f.lower() in b for f in files) and any(k.lower() in b for k in keywords) for b in blocks(text))
...
def score(text, key):
    ...
    for xid, x in key["decoys"].items():
        if any(any(f.lower() in b for f in x["files"]) and any(k.lower() in b for k in x["keywords"])
